...
export function RenderDocumentCard(documents: Record<string, unknown>[], applicationId: string) {
    ...
    let isMultiStepsRow = ["BANK_STATEMENT", "GSTR_FILES"].includes(formPayload.displayName)        
    if(steps.length === 1){
    ...
    const navigateToDocumentSpecificRoute = (documentType: string, route: string, tileInfo: object) => {  
        ...
        if (customFormKey && documentType !== 'MISCELLANEOUS_DOCUMENT') {
            navigate.push(route, {
                state: { formKey: customFormKey, title, description: formattedDescription, applicationId },
            })
        }
        else {
            if (Object.keys(Constants.documentToRouteMapping).includes(documentType) && documentType !== 'MISCELLANEOUS_DOCUMENT')
                navigate.push(route);
            else
    ...
    const getRowType =(index)=>{
        var totalRows = Math.ceil(steps.length / Constants.DOCUMENT_STEPS_BREAK_AFTER_LENGTH)
        var currentRow = Math.ceil((index+1) / Constants.DOCUMENT_STEPS_BREAK_AFTER_LENGTH)
        if(currentRow === 1)
            return "first"
                        ...
                        {steps.map((step,index) => {
                            ...
                            return (
                                <div className="ion-justify-content-center ion-align-items-center" style={{
                                    ...
                                    width: isMultiStepsRow ? `${100 / Constants.DOCUMENT_STEPS_BREAK_AFTER_LENGTH}%` : 'inherit',                                        
                                }}>

                                    {
                                        isMultiStepsRow && (                                                
                                            <h4 className="fs-12 fw-600 ellipse" 
                                                style={{
                                    ...
                                    }
                                    {
                                        ...
                                        <h4 className="fs-12 fw-600 ellipse" 
                                            ...
                                            >
                                            {index === steps.length -1  ? steps[_.size(steps) - 1].title : ""}
                                        </h4>
                                        )
                                    }
